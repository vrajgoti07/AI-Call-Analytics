...
export function TopHeader() {
  ...
  const { data: healthData, isError: isHealthError } = useSystemHealth()
  const isHealthy = !isHealthError && Boolean(healthData?.status)
  const { data: jobsData } = useJobs(undefined, 1, 20)
  const activeJobs = jobsData?.items?.filter((j) => j.status === 'PROCESSING' || j.status === 'QUEUED') || []
  ...
  const activeCompanyName = user?.company?.name || 'Company Workspace'
  ...
  return (
    <header className="h-16 shrink-0 border-b border-[#E5E5E2] bg-white px-4 sm:px-6 flex items-center justify-between z-20">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {user?.role === 'ADMIN' && activeJobs.length > 0 && (
          <Link
            to="/jobs"
        ...
        {user?.role === 'COMPANY' ? (
          /* Read-only Company identity for COMPANY role (no switching allowed) */
          <div
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-[#E5E5E2] bg-[#FAFAF8] text-xs font-medium text-[#17181C] shadow-xs select-none"
            title="Registered Company Workspace"
          >
            <Building2 className="h-3.5 w-3.5 text-[#6D5AE6]" />
            <span className="max-w-[130px] truncate">{activeCompanyName}</span>
          </div>
        ) : (
          ...
          <div className="relative" ref={workspaceRef}>
            <button
              ...
              title="Platform Administration Workspace"
            >
              <Shield className="h-3.5 w-3.5 text-[#6D5AE6]" />
            ...
            {workspaceMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 rounded-xl border border-[#E5E5E2] bg-white p-2 shadow-lg z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="px-2.5 py-1.5 text-[10px] font-semibold text-[#8A8D95] uppercase tracking-wider">
                  Select Workspace ({companies.length})
                </div>
        ...
        <div className="relative" ref={userRef}>
          ...
          {userMenuOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-xl border border-[#E5E5E2] bg-white p-2 shadow-lg z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="px-2.5 py-1.5 border-b border-[#E5E5E2] mb-1">
                <div className="font-semibold text-xs text-[#17181C] truncate">{user?.full_name}</div>
                <div className="text-[11px] truncate text-[#8A8D95]">{user?.email}</div>
                <div className="mt-1.5 inline-block text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-[#F2F0FD] text-[#5844D6] font-semibold">
                  {user?.role}
                </div>
              </div>
