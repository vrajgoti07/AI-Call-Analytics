...
def get_job(
    ...
    # If COMPANY user, ensure job belongs to their company
...
@router.get(
    ...
    summary="List background processing jobs with pagination (Admin only)",
)
def list_jobs(
    call_id: uuid.UUID | None = Query(default=None, description="Filter jobs by call ID"),
    company_id: uuid.UUID | None = Query(default=None, description="Filter jobs by company ID"),
    page: int = Query(default=1, ge=1, description="Page number"),
    page_size: int = Query(default=20, ge=1, le=100, description="Items per page"),
    admin_user: User = Depends(require_admin),
    db: Session = Depends(get_db),
) -> JobListResponse:
    """List historical and active processing jobs (strictly Admin-only)."""
    if call_id:
